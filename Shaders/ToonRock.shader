Shader "ToonURP/ToonRock"
{
    Properties
    {
        ...
        [KWEnum(ShadingMode, CelShading, _CELLSHADING, PBRShading, _PBRSHADING)] _EnumShadingMode ("模式", float) = 0
        ...
        // Feature
        [Main(FeatureMode, _, off, off)] _FeatureGroup("特性", float) = 0
        [KWEnum(FeatureMode, SnowRock, _SNOWROCK, GrassRock, _GRASSROCK)] _EnumFeatureMode ("特性", float) = 0
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 0)] _CustomVector1 ("雪颜色", Color) = (1,1,1,1)
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 0)] _CustomFloat1 ("雪线（世界坐标）", Float) = 0.5
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 1)] _CustomVector2 ("草地岩石颜色", Color) = (1,1,1,1)
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 1)] _CustomFloat2 ("草地缩放", Range(0,1)) = 0.9
        [Tex(FeatureMode_GRASSROCK)] _CustomMap1("草地贴图", 2D) = "white" {}
